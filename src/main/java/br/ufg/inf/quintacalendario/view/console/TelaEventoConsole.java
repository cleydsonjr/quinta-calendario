...
public class TelaEventoConsole extends AbstractTelaCabecalho implements TelaInicial {
    ...
    private void listarPorDescricao() {
        String descricaoEvento = getEntradaConsole().pergunteString("Digite a "
                + "descricão do evento", true);
        EventosController eventosController = new EventosController();
        List<Evento> eventos = eventosController.listar(descricaoEvento);
        if (eventos.isEmpty()) {
            System.out.println("Não existem eventos cadastrados com essa "
                    + "descrição");
        } else {
            eventos.stream().forEach(x -> System.out.println(x.getId() + " - "
                    + x.getTitulo()));
        }
    }

    public void listarPorPeriodo() {
        String dataInicial = getEntradaConsole().pergunteString("Digite a data "
                + "inicial, no formato dd/MM/YYYY", true);
        String dataFinal = getEntradaConsole().pergunteString("Digite a data "
                + "final, no formato dd/MM/YYYY", true);

        EventosController eventoController = new EventosController();
        List<Evento> eventos = eventoController.listarPorPeriodo(dataInicial,
                dataFinal);
        if (eventos.isEmpty()) {
            System.out.println("Não existe eventos cadastrados no periodo"
                    + " informado");
        } else {
            eventos.stream().forEach(x -> System.out.println(x.getId() + " - "
                    + x.getDescricao()));
    ...
    private void cadastrar() {
        ...
        if (dadosCadastroSaoValidos()) {
            ...
            String titulo = getEntradaConsole().pergunteString("Digite o titulo"
                    + " do evento", true);
            String descricao = getEntradaConsole().pergunteString("Digite a "
                    + "descricão do evento", true);
            String dataInicial = getEntradaConsole().pergunteString("Digite a "
                    + "data inicial do evento, no formato dd/MM/YYYY", true);
            String dataFinal = getEntradaConsole().pergunteString("Digite a "
                    + "data final do evento, no formato dd/MM/YYYY", true);
            ...
            boolean result = eventosController.cadastrar(descricao, titulo,
                    dataInicial, dataFinal, codigoCategoria, codigoRegional,
                    codigoInstituto);
            if (result) {
    ...
    private int selecionarCodigoRegional() {
        ...
        do {
            Integer codigo;
            regionais.stream().forEach(x -> System.out.println(x.getId() + " - "
                    + x.getNome()));
            codigo = getEntradaConsole().pergunteInteiro("Digite o codigo da "
                    + "regional do evento");
    ...
    private int selecionarCodigoInstituto() {
        ...
        do {
            ...
            institutos.stream().forEach(x -> System.out.println(x.getId() +
                    " - " + x.getNome()));
            codigo = getEntradaConsole().pergunteInteiro("Digite o codigo do"
                    + " instituto do evento");
    ...
    private int selecionarCodigoCategoria() {
        ...
        do {
            Integer codigo;
            categorias.stream().forEach(x -> System.out.println(x.getId() +
                    " - " + x.getNome()));
            codigo = getEntradaConsole().pergunteInteiro("Digite o codigo da"
                    + " categoria do evento");
    ...
    private boolean dadosCadastroSaoValidos() {
        ...
        if (regionais == null || regionais.isEmpty()) {
            System.out.println("Não existem regionais cadastradas, efetue o seu"
                    + " cadastro antes de prosseguir");
            result = false;
        }

        if (institutos == null || institutos.isEmpty()) {
            System.out.println("Não existem institutos cadastrados, efetue o"
                    + " seu cadastro antes de prosseguir");
            result = false;
        }

        if (categorias == null || categorias.isEmpty()) {
            System.out.println("Não existem categorias cadastradas, efetue o"
                    + " seu cadastro antes de prosseguir");
            result = false;
        }
